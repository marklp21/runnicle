    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      ...
      {filteredEvents.length === 0 ? (
        <div className="text-center py-20 rounded-xl border border-zinc-200 bg-zinc-50">
          ...
          <h3 className="text-zinc-800 font-bold">No events found</h3>
          ...
          {filteredEvents.map((ev, idx) => (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-base font-bold text-zinc-900 tracking-tight">{ev.title}</h3>
                  
                  <div className="flex flex-wrap gap-x-4 gap-y-1.5 text-xs text-zinc-500 mt-2 font-medium">
                    <span>{ev.date}</span>
                    <span>•</span>
                    <span className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-orange-500" /> Bacolod City</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto border-t border-zinc-200 pt-3 sm:pt-0 sm:border-0 justify-between sm:justify-end">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded bg-orange-500 text-white px-4 py-2 text-[10px] font-black uppercase tracking-wider hover:bg-orange-600 transition-colors cursor-pointer shadow-md shadow-orange-500/10"
                >
                  Go to Program
