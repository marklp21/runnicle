...
export const App: React.FC = () => {
  ...
  const getNavbarActiveTab = () => {
    ...
    if (['store', 'product-details', 'cart', 'checkout', 'order-confirmation'].includes(page)) return 'Store';
    if (['coaching', 'book-consultation'].includes(page)) return 'Coaching';
    if (page === 'training') return 'Training';
    if (page === 'gallery') return 'Gallery';
    if (['news', 'article-details'].includes(page)) return 'News';
  ...
  const handleTabChange = (tab: string) => {
    ...
    else if (tab === 'Store') setPage('store');
    else if (tab === 'Coaching') setPage('coaching');
    else if (tab === 'Training') setPage('training');
  ...
  return (
    <div className="flex flex-col min-h-screen bg-black text-zinc-300 antialiased selection:bg-white selection:text-black">
      ...
      {cartTotalItems > 0 && page !== 'cart' && page !== 'checkout' && page !== 'order-confirmation' && (
        <div 
          ...
          className="fixed bottom-6 right-6 z-40 flex h-14 w-14 items-center justify-center rounded-full bg-white text-black shadow-2xl hover:scale-105 transition-transform cursor-pointer"
        >
          <ShoppingBag className="h-6 w-6" />
          <span className="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-zinc-950 text-xs font-black text-white border border-white/20">
            {cartTotalItems}
          </span>
        </div>
      )}
          ...
          >
            ...
            {page === 'coaching' && (
              <div className="py-16 md:py-24 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto mb-16">
                  ...
                  <h1 className="mt-6 font-display text-4xl font-black leading-tight text-white sm:text-5xl md:text-6xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
                  {/* Coach 1 */}
                  <div className="rounded-xl border border-zinc-800 bg-[#0c0c0c] overflow-hidden p-6 hover:border-zinc-700 transition-all flex flex-col justify-between">
                    <div>
                      <img
                        src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&w=300&h=300&q=80"
                        alt="Coach Dan"
                        className="h-32 w-32 rounded-full object-cover border-2 border-zinc-700 mx-auto mb-6"
                      />
                      <h3 className="text-xl font-bold text-white text-center">Coach Dan</h3>
                      ...
                      <div className="border-t border-zinc-900 my-4 pt-4 text-xs space-y-2 text-zinc-400">
                        <p className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-white flex-shrink-0" /> Over 15 years running experience</p>
                        <p className="flex items-center gap-2"><Award className="h-4 w-4 text-white flex-shrink-0" /> Qualified sub-2:30 marathoner</p>
                        <p className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-white flex-shrink-0" /> Focuses on aerobic engine & mindset</p>
                      </div>
                    </div>
                    <button
                      onClick={() => {
                      ...
                      className="w-full rounded-md bg-white py-3.5 mt-4 text-xs font-bold text-black hover:bg-zinc-200 transition-all cursor-pointer uppercase tracking-wider"
                    >
                      Book Consultation
                    </button>
                  </div>
                  ...
                  <div className="rounded-xl border border-zinc-800 bg-[#0c0c0c] overflow-hidden p-6 hover:border-zinc-700 transition-all flex flex-col justify-between">
                    <div>
                      <img
                        ...
                        className="h-32 w-32 rounded-full object-cover border-2 border-zinc-700 mx-auto mb-6"
                      />
                      <h3 className="text-xl font-bold text-white text-center">Coach Clara</h3>
                      ...
                      <div className="border-t border-zinc-900 my-4 pt-4 text-xs space-y-2 text-zinc-400">
                        <p className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-white flex-shrink-0" /> Specialized in biomechanics & core</p>
                        <p className="flex items-center gap-2"><Award className="h-4 w-4 text-white flex-shrink-0" /> Completed multiple 100-mile trail races</p>
                        <p className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-white flex-shrink-0" /> Focuses on posture, agility & recovery</p>
                      </div>
                    </div>
                    <button
                      onClick={() => {
                  ...
                  <div className="rounded-xl border border-zinc-800 bg-[#0c0c0c] overflow-hidden p-6 hover:border-zinc-700 transition-all flex flex-col justify-between">
                    <div>
                      <img
                        ...
                        className="h-32 w-32 rounded-full object-cover border-2 border-zinc-700 mx-auto mb-6"
                      />
                      <h3 className="text-xl font-bold text-white text-center">Coach Alex</h3>
                      <p className="text-[10px] text-zinc-500 font-extrabold uppercase text-center tracking-widest mt-1">V02 Max & Track Speed</p>
                      
                      <div className="border-t border-zinc-900 my-4 pt-4 text-xs space-y-2 text-zinc-400">
                        <p className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-white flex-shrink-0" /> Certified Track & Field athletics coach</p>
                        <p className="flex items-center gap-2"><Award className="h-4 w-4 text-white flex-shrink-0" /> Sub-15 minute 5,000m specialist</p>
                        <p className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-white flex-shrink-0" /> Focuses on lactate threshold & sprints</p>
                      </div>
                    </div>
                    <button
                      onClick={() => {
                ...
                <div className="rounded-xl border border-zinc-800 bg-gradient-to-r from-zinc-950 to-zinc-900 p-8 text-center">
                  <h4 className="text-lg font-bold text-white mb-2">Not sure which coach is right for you?</h4>
                  <p className="text-xs text-zinc-400 mb-6">Take our placement program and schedule a session with our panel organizer.</p>
                  <button
                    onClick={() => {
                      setSelectedCoach('AthleRun Head Coach');
                      setPage('book-consultation');
                    }}
                    className="rounded-full bg-white px-6 py-2.5 text-xs font-bold text-black hover:bg-zinc-200 transition-all cursor-pointer"
                  >
                    Match Me Now
                  </button>
                </div>
            ...
            {page === 'training' && (
              <div className="py-16 md:py-24 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="rounded-xl border border-zinc-800 bg-[#0c0c0c] p-6 md:p-8 max-w-4xl mx-auto mb-12">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    
                    {/* Distance Option */}
                    <div>
                      <label className="block text-xs font-extrabold text-white uppercase tracking-wider mb-3">Target Distance</label>
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
                        {['5K', '10K', 'Half Marathon', 'Full Marathon'].map((dist) => (
                          <button
                            key={dist}
                            type="button"
                            onClick={() => {
                              setPlanDistance(dist);
                              setGeneratedPlan(null);
                            }}
                            className={`rounded border py-3 text-center text-xs font-bold tracking-wider uppercase transition-all cursor-pointer ${
                              planDistance === dist
                                ? 'bg-white text-black border-white'
                                : 'border-zinc-800 text-zinc-400 bg-transparent hover:border-zinc-600 hover:text-white'
                            }`}
                          >
                            {dist}
                          </button>
                    ...
                    {/* Level Option */}
                    <div>
                      <label className="block text-xs font-extrabold text-white uppercase tracking-wider mb-3">Fitness Level</label>
                  ...
                  <button
                    ...
                    className="w-full rounded-md bg-white py-4 text-sm font-black text-black hover:bg-zinc-200 active:scale-98 transition-all duration-200 cursor-pointer uppercase tracking-wider"
                  >
                ...
                <AnimatePresence>
                  {generatedPlan && (
                    ...
                    >
                      <h3 className="text-xl font-black text-white mb-6 text-center tracking-tight flex items-center justify-center gap-2">
                      ...
                      <div className="space-y-4">
                        {generatedPlan.map((weekData: { week: string; focus: string; runs: string[] }, idx: number) => (
                          <div
                            key={idx}
                            className="rounded-lg border border-zinc-800 bg-[#080808] p-5 hover:border-zinc-700 transition-all"
                          >
                            <div className="flex flex-col sm:flex-row justify-between sm:items-center border-b border-zinc-900 pb-3 mb-4 gap-2">
                              <span className="font-display font-black text-white text-lg uppercase tracking-tight">{weekData.week}</span>
                              <span className="text-xs text-zinc-500 font-bold bg-zinc-950 border border-zinc-900 rounded px-2.5 py-1 uppercase">{weekData.focus}</span>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                              {weekData.runs.map((run: string, rIdx: number) => (
                                <div key={rIdx} className="rounded bg-[#0c0c0c] border border-zinc-900 p-3 text-xs text-zinc-300 font-medium">
                                  {run}
                                </div>
                              ))}
                            </div>
                      ...
                      <div className="mt-8 text-center">
                        <button
                          ...
                          className="rounded-full bg-white px-8 py-3 text-xs font-bold text-black hover:bg-zinc-200 transition-all cursor-pointer uppercase tracking-wider"
                        >
      ...
      <Footer onPlatformClick={(item) => {
        if (['Coaching Portal'].includes(item)) setPage('coaching');
        else if (['Event Calendar'].includes(item)) setPage('events');
        else if (['Leaderboards'].includes(item)) {
          setSelectedEvent(null);
          setPage('events');
        }
        else if (['Shop Equipment'].includes(item)) setPage('store');
        else {
      ...
      >
        {selectedPost && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 overflow-hidden">
            {/* Post image */}
            <div className="relative aspect-square rounded-lg overflow-hidden bg-black border border-zinc-900 flex items-center justify-center">
              <img
            ...
            <div className="flex flex-col h-full justify-between max-h-[500px]">
              ...
              <div className="border-b border-zinc-900 pb-4 mb-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  />
                  <div>
                    <span className="text-xs font-bold text-white block">@{selectedPost.username}</span>
              ...
              <div className="flex-1 overflow-y-auto mb-4 pr-1.5 space-y-3 max-h-[220px]">
                {selectedPost.comments.length === 0 ? (
                  ...
                  selectedPost.comments.map((c, idx) => (
                    <div key={idx} className="text-[11px] font-medium leading-relaxed bg-zinc-950/40 p-2.5 rounded border border-zinc-900">
              ...
              <div className="border-t border-zinc-900 pt-4 mt-auto">
                <div className="flex items-center gap-3.5 text-xs text-white mb-4">
                  <button
                    onClick={() => handleToggleLike(selectedPost.id)}
                    className="flex items-center gap-1.5 hover:text-red-500 transition-colors font-bold cursor-pointer"
                  >
                    <Heart className={`h-4.5 w-4.5 ${
                      (selectedPost as any).hasLiked ? 'fill-red-500 stroke-red-500' : 'text-zinc-400 hover:text-red-500'
                    }`} />
                    <span>{selectedPost.likes} Likes</span>
                  </button>
                  <span className="text-zinc-600 font-medium">|</span>
                  <span className="text-zinc-400 font-medium">{selectedPost.comments.length} Comments</span>
                </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 rounded border border-zinc-800 bg-[#080808] px-3 py-2 text-xs text-white placeholder-zinc-700 focus:border-white focus:outline-none"
                  />
                  <button
                    onClick={() => handleAddComment(selectedPost.id)}
                    className="rounded bg-white px-4 py-2 text-xs font-bold text-black hover:bg-zinc-200 transition-colors uppercase tracking-wider cursor-pointer"
                  >
                    Post
                  </button>
                </div>
