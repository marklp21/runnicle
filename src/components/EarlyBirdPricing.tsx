...
export const EarlyBirdPricing: React.FC<EarlyBirdPricingProps> = ({ event }) => {
  ...
  return (
    <section className="relative bg-white py-16 sm:py-24 px-4 sm:px-6 lg:px-8 select-none font-sans slashed-zero [font-variant-numeric:slashed-zero]">
      ...
      {/* Orange Radial Glow smaller for mobile viewports (top-[135px], h-[380px], w-[84vw]) without altering desktop (sm:top-[160px], sm:h-[620px]) */}
      <div 
        className="absolute top-[135px] sm:top-[160px] left-1/2 -translate-x-1/2 w-[84vw] sm:w-full sm:max-w-2xl h-[380px] sm:h-[620px] pointer-events-none z-0"
        style={{
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center">
        ...
        <motion.div
          ...
          className="relative max-w-2xl mx-auto bg-white rounded-2xl border border-zinc-300 p-3.5 sm:p-8 text-left overflow-hidden shadow-xs z-10"
        >
          {/* Table Header */}
          <div className="grid grid-cols-3 gap-1 sm:gap-4 pb-2 text-[11px] sm:text-sm font-sans font-semibold text-zinc-900">
            <div>Distance</div>
          ...
          <div>
            {referenceRows.map((row) => (
              <div
                key={row.label}
                className="grid grid-cols-3 gap-1 sm:gap-4 py-3.5 sm:py-4.5 items-center"
              >
                {/* Distance Column */}
                ...
                <div className="flex items-center justify-center gap-1 sm:gap-3">
                  <span className="font-mono text-[10px] sm:text-sm text-zinc-500 line-through font-medium slashed-zero whitespace-nowrap">
                    {row.origTicket}
                  </span>
                  <span className="inline-flex items-center justify-center rounded-full border border-[#FF4400] text-[#FF4400] bg-[#FFF3EB] px-1.5 sm:px-4 py-0.5 sm:py-1 text-[10px] sm:text-sm font-bold font-mono tracking-wider slashed-zero whitespace-nowrap">
                    {row.earlyTicket}
                  </span>
                </div>
